...
type __X_IF_FIELD_Pointer_THEN_AMPERSAND_ENDIF_ACTION_FIELD_Name_ENDACTION__ struct{} /// {{end}}
...
/// {{define "body-none"}}{{.Receiver}} .Called( {{template "args" .}} ) {{end}}
/// {{define "body-one"}}return {{.Receiver}} .Called( {{template "args" .}} ) {{$t := (index .ResultsFlat 0).Type}} {{if ne $t "interface{}"}} . ( {{$t}} ) {{end}} {{end}}
/** {{define "body-many"}} {
	var result = {{.Receiver}} .Called( {{template "args" .}} )
