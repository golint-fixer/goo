...
var macroFuncs = map[*regexp.Regexp][]byte{
	regexp.MustCompile(`ACTIONX_(.+)_END`):                                    []byte("{{$1}}"),
	regexp.MustCompile(`ACTION_([[:alnum:]]+)`):                               []byte("{{$1}}"),
	regexp.MustCompile(`BLOCKX_(.+)_BEGIN_(.+)_END`):                          []byte(`{{block "$1" $2}}`),
	regexp.MustCompile(`BLOCK_([[:alnum:]]+)_([[:alnum:]]+)`):                 []byte(`{{block "$1" $2}}`),
	regexp.MustCompile(`FIELD_([[:alnum:]]+)`):                                []byte(".$1"),
	regexp.MustCompile(`GROUPX_(.+)_END`):                                     []byte("($1)"),
	regexp.MustCompile(`GROUP_([[:alnum:]]+)`):                                []byte("($1)"),
	regexp.MustCompile(`IFEX_(.+)_THEN_(.+)_ELSE_(.+)_END`):                   []byte("{{if $1}}$2{{else}}$3{{end}}"),
	regexp.MustCompile(`IFE_([[:alnum:]]+)_([[:alnum:]]+)_([[:alnum:]]+)`):    []byte("{{if $1}}$2{{else}}$3{{end}}"),
	regexp.MustCompile(`IFX_(.+)_THEN_(.+)_END`):                              []byte("{{if $1}}$2{{end}}"),
	regexp.MustCompile(`IF_([[:alnum:]]+)_([[:alnum:]]+)`):                    []byte("{{if $1}}$2{{end}}"),
	regexp.MustCompile(`INITX_([[:alnum:]]+)_(.+)_END`):                       []byte("$$$1 := $2"),
	...
	regexp.MustCompile(`OMITX_.+_END`):                                        []byte(""),
	regexp.MustCompile(`OMIT_[[:alnum:]]+`):                                   []byte(""),
	regexp.MustCompile(`RANGEEX_(.+)_BEGIN_(.+)_ELSE_(.+)_END`):               []byte("{{range $1}}$2{{else}}$3{{end}}"),
	regexp.MustCompile(`RANGEE_([[:alnum:]]+)_([[:alnum:]]+)_([[:alnum:]]+)`): []byte("{{range $1}}$2{{else}}$3{{end}}"),
	regexp.MustCompile(`RANGEX_(.+)_BEGIN_(.+)_END`):                          []byte("{{range $1}}$2{{end}}"),
	regexp.MustCompile(`RANGE_([[:alnum:]]+)_([[:alnum:]]+)`):                 []byte("{{range $1}}$2{{end}}"),
	regexp.MustCompile(`RAWX_(.+)_END`):                                       []byte("`$1`"),
	...
	regexp.MustCompile(`STRINGX_(.+)_END`):                                    []byte(`"$1"`),
	regexp.MustCompile(`STRING_([[:alnum:]]+)`):                               []byte(`"$1"`),
	regexp.MustCompile(`TEMPLATEX_(.+)_BEGIN_(.+)_END`):                       []byte(`{{template "$1" $2}}`),
	...
	regexp.MustCompile(`WITHEX_(.+)_BEGIN_(.+)_ELSE_(.+)_END`):                []byte("{{with $1}}$2{{else}}$3{{end}}"),
	regexp.MustCompile(`WITHE_([[:alnum:]]+)_([[:alnum:]]+)_([[:alnum:]]+)`):  []byte("{{with $1}}$2{{else}}$3{{end}}"),
	regexp.MustCompile(`WITHX_(.+)_BEGIN_(.+)_END`):                           []byte("{{with $1}}$2{{end}}"),
